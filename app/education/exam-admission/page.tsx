...
import { getAustraliaNews } from '@/lib/wordpress';

// ISR: Revalidate every 10 minutes
export const revalidate = 600;

async function getAustraliaData() {
  try {
    const posts = await getAustraliaNews(12);
    return posts;
  } catch (error) {
...
export default async function AustraliaPage() {
  const articles = await getAustraliaData();
        ...
        <div className="bg-gray-50 dark:bg-gray-800 py-4">
          <div className="container mx-auto px-4">
            <nav className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span className="text-gray-900 dark:text-white">Exam/Admission</span>
            </nav>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-orange-600 to-orange-700 text-white py-16">
          <div className="container mx-auto px-4">
            <div className="text-center">
              <h1 className="text-4xl md:text-6xl font-bold mb-4">Exam & Admission</h1>
              <p className="text-xl opacity-90 max-w-2xl mx-auto">
                Comprehensive guides on entrance exams, admission requirements, 
                test preparation, and application processes.
              </p>
            </div>
          </div>
        </div>
          ...
          {articles.length > 0 ? (
            ...
            <div className="text-center py-16">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
                No Exam/Admission Guides Available
              </h2>
              <p className="text-gray-600 dark:text-gray-300 mb-8">
                We're working to bring you the latest exam and admission guides. Please check back soon.
              </p>
              <Button asChild>
